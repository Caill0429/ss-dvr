...
class TrainerACE:
    ...
    def train(self):
        ...
        # self.draw_points_3d(self.training_buffer['target_coord'], self.training_buffer['cluster_labels'][:, 0])
    ...
    def create_training_buffer(self):
        ...
        with torch.no_grad():
            ...
            while buffer_idx < self.options.training_buffer_size:
                ...
                for image_B1HW, image_mask_B1HW, coord_BHW3, gt_pose_B44, gt_pose_inv_B44, intrinsics_B33, intrinsics_inv_B33, _, _ in training_dataloader:
                    # Copy to device.
    ...
    def train_clusters(self):
        ...
        for batch_start in range(0, self.options.training_buffer_size, self.options.batch_size):
            ...
            features = features[None, None, ...].view(-1, 16, 32, channels).permute(0, 3, 1, 2)
            cluster_labels = self.training_buffer['cluster_labels'][random_batch_indices].contiguous()
            cluster_labels = cluster_labels.permute(1, 0).to(torch.int64)
            with autocast(enabled=self.options.use_half):
                pred_labels = self.regressor.get_cluster_labels(features)

            pred_labels = pred_labels.permute(0, 2, 3, 1).flatten(0, 2).unsqueeze(-1).float()
            pred_labels = pred_labels.permute(2, 1, 0)
            cluster_loss = self.cls_loss(pred_labels, cluster_labels)

            self.optimizer.zero_grad()
            self.scaler.scale(cluster_loss).backward(retain_graph=True)
            self.scaler.step(self.optimizer)
            self.scaler.update()

            loss.append(cluster_loss.item())

        _logger.info(f'Loss_cluster: {sum(loss) / len(loss):.5f} ')
    ...
    def cal_val(self):
        ...
        for i in range(self.options.feature_clusters):
            for j in range(self.options.spatial_clusters):
                ...
                feat_mean = torch.mean(self.training_buffer['features'][mask.squeeze(-1)], dim=0)
                feat_dist = torch.norm(self.training_buffer['features'][mask.squeeze(-1)] - feat_mean, dim=1)
                # feat_dist = feat_dist * feat_dist
                average_feat = torch.mean(feat_dist)
                feat_list.append(average_feat)
                feat_max.append(torch.max(feat_dist))

                dist_mean = torch.mean(self.training_buffer['target_coord'][mask.squeeze(-1)], dim=0)
                dist_dist = torch.norm(self.training_buffer['target_coord'][mask.squeeze(-1)] - dist_mean, dim=1)
                # dist_dist = dist_dist * dist_dist
                average_dist = torch.mean(dist_dist)
                dist_list.append(average_dist)
                dist_max.append(torch.max(dist_dist))

        print(torch.max(torch.stack(feat_max)))
        print(torch.mean(torch.stack(feat_list)))
        print(torch.max(torch.stack(dist_max)))
        print(torch.mean(torch.stack(dist_list)))
        print()
        ...
        dist_max = []
        for i in range(self.options.feature_clusters):
            mask = self.training_buffer['feature_labels'] == i
        ...
        feat_max = []
        dist_max = []
        for i in range(self.options.spatial_clusters):
            mask = self.training_buffer['spatial_labels'] == i
